...
namespace webprintDesigner.Helpers
{
    ...
    public class FileUpload : INotifyPropertyChanged
    {
        ...
        public void Upload(int pProductID, int pFileType, int pCustomerID)
        {
            ...
            cancel = false;
            //commenting to remove resize functionality
        ...
        private void CheckFileOnServer()
        {
            ...
            ub.Query = string.Format("{1}filename={0}&GetBytes=true&ProductID={2}&FileType={3}&CustomerID={4}", File.Name, string.IsNullOrEmpty(ub.Query) ? "" : ub.Query.Remove(0, 1) + "&",ProductID,FileType, CustomerID);
        ...
        public void UploadFileEx()
        {
            ...
            ub.Query = string.Format("{3}filename={0}&StartByte={1}&Complete={2}&ProductID={4}&FileType={5}&IsOverWrite={6}&CustomerID={7}", File.Name, BytesUploaded, complete, string.IsNullOrEmpty(ub.Query) ? "" : ub.Query.Remove(0, 1) + "&", ProductID,FileType,isOverwriting.ToString(),CustomerID);
