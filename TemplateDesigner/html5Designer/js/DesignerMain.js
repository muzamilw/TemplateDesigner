(function (global) {
    ...
    $("#btnzoomin").click(function (event) {
        ...
        var g = new fabric.Group(canvas.getObjects());
        g.setActive(true);
        g.scale(4);
        canvas.renderAll();
    ...
    function onObjectSelected(e) {
        ...
        if (selectedObject && selectedObject.type === 'text') {
            $("#text").text(selectedObject.text);
            //this.value = activeObject.text;
        }

        $("#output").text(selectedObject.getWidth());
        $("#output").text(selectedObject.maxWidth);
    }
...
//        };
    ...
    document.getElementById('commands').onclick = function (ev) {
        ...
        switch (className) {
            ...
            case 'image1':
                fabric.Image.fromURL('assets/Imageerror.png', function (image) {
                    image.set({
                        left: left,
                        top: top,
                        angle: angle
