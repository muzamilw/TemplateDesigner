...
(function () {
    ...
    */
    setBackgroundImage: function (url, callback, options) {
        if(url != null) { // updated by saqib to handle empty image
            fabric.util.loadImage(url, function (img) {
        ...
        if (activeGroup) {
            ...
            if (D1AG) {
                ...
                if (!lockedObjectFound) {
                    ...
                    animatedcollapse.hide("DivAlignObjs");
                }
            }
            //animatedcollapse.hide(['textPropertPanel', 'DivAdvanceColorPanel', 'DivColorPallet', 'ShapePropertyPanel', 'ImagePropertyPanel', 'UploadImage', 'quickText', 'addImage', 'addText','divPositioningPanel']);
            //        if (D1CZL == 0)
            //        {
    ...
    */
    _scaleObject: function (x, y, by) {
        ...
        if( target.type != "text") {
            ...
            if(newScaleY > 0.02 || newScaleX > 0.02 )
...
(function (global) {
    ...
    */
    ...
    _render: function (context) {
      ...
      //  context.textAlign = this.textAlign;   // commented by saqib to adjust line spacing
...
//            xPos = this.width / 2;
        ...
        this.wrapText(context, this.text, xPos, -this.height / 2 +( this.fontSize *  this.lineHeight ) -( this.fontSize * 0.21), this.maxWidth, this.lineHeight * this.fontSize , this.height, this.charSpacing,this.textAlign,this.fontSize);
        this.setCoords();
    },

    wrapText: function (context, text, x, y, maxWidth, lineHeight, BoxHeight,charSpacing,textAlign,fontSize) {
        //edited by saqib ali to solve new line logic 
        charSpacing =parseFloat(charSpacing);// parseInt(charSpacing);
        ...
        for (var nz = 0; nz < sentence.length; nz++) {
            ...
            for (var n = 0; n < words.length; n++) {
                ...
                var testWidth = metrics.width + spacingWidth;
                    ...
                    if (wordCount == 0 || (wordCount) < BoxHeight) {
                        wordCount += 1;
                            ...
                            if(textAlign == "left") {
                                ...
                                var m = context.measureText(line.substring(0,line.length-1));
                                var lc = (line.length-2) * charSpacing ;
                                if(line[line.length-1] != " ")  {
                                    m = context.measureText(line);
                                    lc = (line.length-1) * charSpacing ;
                                } 
                                startingX = -(m.width/2)  - (lc/2);  
                                ...
                                startingX = maxWidth/2 - (m.width)  - (lc);
                            } 
                            for(var i = 0 ; i < line.length;i++) {
                                context.fillText(line[i], startingX, y);
                                var metrics2 = context.measureText(line[i]);
                                startingX =startingX + charSpacing +metrics2.width  ;
                            }
                        line = words[n] + " ";
