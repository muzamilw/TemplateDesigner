...
var Cufon = (function () {
    ...
    function process(obj, font, text, style, options, node, el) {
    ...
    function replaceElement(obj, el, options) {
        ...
        for (var node = attach(el, options).firstChild; node; node = nextNode) {
            ...
            if (text === '') continue;
            var processed = process(obj, font, text, style, options, node, el);
    ...
    var engines = {}, fonts = {}, defaultOptions = {
      ...
      || (fabric.window.jQuery && function (query) { return jQuery(query); }) //f52
    ...
    api.replace = function (elements, options, ignoreHistory) {
        ...
        if (!options.engine) return api; // f54
    ...
    api.replaceElement = function (obj, el, options) {
        options = merge(defaultOptions, options);
        if (typeof options.textShadow == 'string' && options.textShadow)
            options.textShadow = CSS.textShadow(options.textShadow);
        return replaceElement(obj, el, options);
    };
...
Cufon.registerEngine('canvas', (function () {
    ...
    return function (obj, font, text, style, options, node, el) {

        //width = Math.max(maxWidth, width);
        var size = style.getSize('fontSize', font.baseSize);
        var owidth = size.convertFrom(parseFloat(obj.maxWidth));
        ...
        for (var i = 0, l = chars.length; i < l; ++i) {
            if (chars[i] === '\n') {
                ...
                lineWidths.push(owidth);
                width = 0;
                continue;
            }


            //new logic of defining new lines based on wrapping
            ...
            width += lastWidth = font.w; // Number(glyph.w || font.w) + letterSpacing;
            linewidth += lastWidth;
        ...
        width = owidth;
        var oheight = size.convertFrom(parseFloat(obj.maxHeight));
        //lastWidth = width;
        ...
        g.save();
        g.scale(scale, scale);
        //alert(totalLineHeight);


        g.translate(-owidth / 2, -oheight / 2 - font.ascent - ((font.ascent / 5) * options.lineHeight));   //size.convertFrom((_height * lines) + totalLineHeight)
        ...
        function renderText(color) {
            ...
            for (var i = 0, l = chars.length; i < l; ++i) {
                ...
                if (font.w * linecharindex > lineWidths[lineNum]) {
                    var topOffset = -font.ascent - ((font.ascent / 5) * options.lineHeight);
                    if (options.textAlign === 'right') {
                        g.translate(-left, topOffset);
                        //g.translate(lineOffsets[lineNum], 0);
...
(function () {
    ...
    */
    __onMouseMove: function (e) {
        ...
        if (groupSelector !== null) {
            ...
            //  added by saqib to solve the locked object movemenet issue 28 sept 
            if (this._currentTransform.target.IsPositionLocked != true) {
                ...
                if (this._currentTransform.action === 'rotate') {
                    ...
                    // added to remove scale from rotate
                } else if (this._currentTransform.action === 'scaleObj') {
    ...
    _setupCurrentTransform: function (e, target) {
        ...
        if (corner = target._findTargetCorner(e, this._offset)) {
            ...
            if (corner === 'ml' || corner === 'mr')
                ...
                action = 'scaleY'
            else if (corner === 'br' || corner === 'tl' || corner === 'bl') {
                if (target.type != "text")
                    action = 'scaleObj'
            } else
                action = 'rotate'
        ...
        if (target.type != "text") {
            if (!by) {
                ...
                if (t.ex > x) {
                    ...
                    var lastLen1 = sqrt(pow(t.left - offset.left, 2)),
                    curLen1 = sqrt(pow(t.left - offset.left, 2));
                    var lea = sqrt(pow(x - t.ex, 2));
                    templ = lastLen1 + curLen1 - lea / 0.65;
                    diffx = templ;
                }
                target.lockScalingX || target.set('scaleX', (diffx / target.width));
                ...
                target.lockScalingY || target.set('scaleY', (diffy / target.height));
                //target.lockScalingX || target.set('scaleY', (t.scaleY * curLen/lastLen));
                dif = target.getHeight() - oldH;
                dif = dif / 2
                target.top = target.top + dif;
                //this._setCursor('s-resize');
            }
        }
        else {
            if (!by) {
                var diffx = lastLen + curLen;
                var diffy = lastLen + curLen;
                target.set('width', diffx);
                target.set('maxWidth', diffx);
                target.set('height', diffy);
                target.set('maxHeight', diffy);
            }
            else if (by === 'x' && !target.lockUniScaling) {
                var diffx = lastLen + curLen;
                if (t.ex > x) {
                    diffx = lastLen + curLen + x - t.ex;
                }
                // dited by saqib for sorting zoom issue 
                if (D1CZL != 0) {
                    diffx = diffx * (1 / D1CS);
                }
                ...
                //var diffy = lastLen+ curLen+ (y- t.ey)/2;
                // edited by saqib for sorting zoom issue 
                var diffy = lastLen + curLen;
                if (t.ey > y) {
                    diffy = lastLen + curLen + y - t.ey;
    ...
    */
    deactivateAllWithDispatch: function () {
        var D1AO = this.getActiveGroup() || this.getActiveObject();
        if (D1AO) {
            this.fire('before:selection:cleared', { target: D1AO });
        }
        this.deactivateAll();
        if (D1AO) {
...
(function (global) {
    ...
    */
    stateProperties: ('top left width height scaleX scaleY flipX flipY ' +
                      ...
                      'strokeWidth fillRule borderScaleFactor transformMatrix text ' +
                      'selectable').split(' '),
    ...
    // cornerColor:              'rgba(102,153,255,0.5)', changed by saqib from blue to orange for group corners
    cornerColor: 'rgba(255,165,0,1)',
    fill: 'rgb(0,0,0)',
    ...
    */
    setCoords: function () {
        ...
        var tr = {
            //saqib 
            //         x: tl.x + (this.currentWidth * cosTh) + 12,
            //        y: tl.y + (this.currentWidth * sinTh) -10
            x: tl.x + (this.currentWidth * cosTh),
            y: tl.y + (this.currentWidth * sinTh)
        };
        var br = {
            //        x: tr.x - (this.currentHeight * sinTh) -12,saqib 
            //        y: tr.y + (this.currentHeight * cosTh) +10
        ...
        var mr = {
            //        x: tr.x - (this.currentHeight/2 * sinTh) -12,saqib
            //        y: tr.y + (this.currentHeight/2 * cosTh) +10 
    ...
    */
    drawBorders: function (ctx) {
        ...
        //         ctx.strokeRect(
        //        ~ ~(-(w ) - padding) + 0.5, // offset needed to make lines look sharper
        //        ~ ~(-(h ) - padding) + 0.5,
        //        ~ ~(w + padding2),
        //        ~ ~(h + padding2)
        //      );
    ...
    */
    drawCorners: function (ctx) {
        ...
        //        if(this.type == "image") {
        //             ctx.moveTo(_left+15, _top+10);
        //            ctx.lineTo(_left + 31, _top - 25);
        //        }else {
        //            ctx.moveTo(_left+5, _top+5);
        //            ctx.lineTo(_left + 16, _top - 10);
        //        }
        //        

        //variables for drwaing arc
        ...
        //        ctx.fillStyle = this.cornerColor;
        //        ctx.beginPath();
        //        //ctx.arc(_left + 15, _top - 10, size, 0, 2 * Math.PI, false);
        //        if(this.type == "image")
        //        {
        //            ctx.arc(_left + 30, _top - 20, size *1.5, startAngle, endAngle, counterClockwise);
        //        } else 
        //        {
        //            ctx.arc(_left + 15, _top - 10, size/2, startAngle, endAngle, counterClockwise);
        //        }
        //        
        //        ctx.lineWidth = 15;
        //        ctx.fill();
...
(function (global) {
    ...
    */
    _restoreObjectState: function (object) {
        ...
        //  checking shift key group selection issue by saqib

        if (object.left > (this.left + this.width) || object.top > (this.top + this.height)) {
            object.set('left', object.originalLeft);
...
(function (global) {
    ...
    */
    ...
    _render: function (context) {
        ...
        context.font = (this.fontWeight == 'bold' ? 'bold ' : '') + (this.fontStyle == 'italic' ? 'italic ' : '') + this.fontSize + 'px ' + this.fontFamily;
        ...
        this.wrapText(context, this.text, xPos, -this.height / 2 + this.fontSize - this.fontSize * 0.21, this.maxWidth, this.lineHeight * this.fontSize, this.height);
        this.setCoords();
    },

    wrapText: function (context, text, x, y, maxWidth, lineHeight, BoxHeight) {
        //edited by saqib ali to solve new line logic 
        ...
        for (var i = 0; i < sentence.length; i++) {
            ...
            for (var n = 0; n < words.length; n++) {
                ...
                if (testWidth > maxWidth) {
                    ...
                    if (wordCount == 0 || (wordCount) < BoxHeight) {
                        wordCount += 1;
